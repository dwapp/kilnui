                                  ...
                                  Clay_StringSlice text, int font_size)
{
    ...
    if (plen > text.length)
        plen = text.length;
    if (plen > MEASURE_CACHE_PREFIX)
        plen = MEASURE_CACHE_PREFIX;
...
#ifdef KILNUI_HAS_FONTCONFIG
        if (fc_ok)
            path = fc_find_font(fallback_families[i]);
#endif
        if (!path)
            path = fallback_hardcoded[i];
        if (!path)
            continue;
...
void KilnUI_destroy(KilnUI *ctx)
{
    ...
    if (ctx->rect_vbuf)
        SDL_ReleaseGPUBuffer(ctx->gpu, ctx->rect_vbuf);
    if (ctx->rect_ibuf)
        SDL_ReleaseGPUBuffer(ctx->gpu, ctx->rect_ibuf);
    if (ctx->text_vbuf)
        SDL_ReleaseGPUBuffer(ctx->gpu, ctx->text_vbuf);
    if (ctx->text_ibuf)
        SDL_ReleaseGPUBuffer(ctx->gpu, ctx->text_ibuf);
    if (ctx->staging_tbuf)
        SDL_ReleaseGPUTransferBuffer(ctx->gpu, ctx->staging_tbuf);
    ...
    if (ctx->pipeline_shadow)
        SDL_ReleaseGPUGraphicsPipeline(ctx->gpu, ctx->pipeline_shadow);
    if (ctx->pipeline_border)
        SDL_ReleaseGPUGraphicsPipeline(ctx->gpu, ctx->pipeline_border);
...
void KilnUI_set_font_size(KilnUI *ctx, float ptsize)
{
    ...
    if (ctx->cached_font_size == ptsize)
        return;
