...
static void section_badges(void)
{
    ...
    UI_ROW(731, 8)
    {
        if (!UI_Chip(740, "dark-mode", g_chip_a, true))
            g_chip_a = false;
        if (!UI_Chip(741, "typescript", g_chip_b, true))
            g_chip_b = false;
        if (!UI_Chip(742, "open-source", g_chip_c, true))
            g_chip_c = false;
...
int main(int argc, char *argv[])
{
    ...
    if (!KilnUI_init(&ctx, "KilnUI Design System", 1100, 760, font, 14))
        return 1;
